   ...
   for (i=0;i<r;i++)//从第0行开始 
    {
        for (k = 0;k < c;k++)//从第0列开始 
         scanf("%d", &a[i][k]);//二维数组嵌套输入初始方式  就整形初始化 字符型不好初始化 
    }
    scanf("%d",h);
    getchar();
    struct num n[100];//来分别记录每一行数据 i代表每一行  //两个地方改进 一 我并不是只每一次 而是要用一个数组记录，最后再打出来 
    int x,y;//记录数字 
    for (i=0;i<r;i++)
  {
   for (k=0;k<c;k++)
...
else if(strcmp(scmd,"SC")==0)//交换一列二列 
{scanf("%d%d",&x,&y);
	for (i= 0; i<r;i++)  
{ 
	    for ( k=0;k<c;k++)
...
else if(strcmp(scmd,"IC")==0)//y前加一列 全为0 
{  scanf("%d",&x);
   c++;
	for (i= 0; i<r;i++)  
{ 
